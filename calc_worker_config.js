...
const urList = {};
const UR_E = W.E;
const UR_LN10 = W.TEN.ln();
const UR_RADIANS_PER_DEGREE = W.RADIANS_PER_DEGREE;
...
function urToBigInt(ur) {
    if (ur.digitsRequired() === 0) {
        ...
        if (asBI === null) {
            ...
            if (!ur.approxEquals(W.newN(asBI), -1000)) {
                return null;
            }
        }
        return asBI;
            ...
            try {
                switch (token) {
                    ...
                    case "sinh":
                        ...
                        stack.push(getDivide(getSub(getPowUR(UR_E, arg0), getPowUR(UR_E, getNegate(arg0))), W.TWO));
                        break;
                    case "cosh":
                        if (!CONFIG_HYP) {
                            throw new Error("Unsupported Operation: cosh");
                        }
                        stack.push(getDivide(getAdd(getPowUR(UR_E, arg0), getPowUR(UR_E, getNegate(arg0))), W.TWO));
                        break;
                    case "tanh": {
                        if (!CONFIG_HYP) {
                    ...
                    case "asinh":
                        ...
                        stack.push(getLn(getAdd(arg0, getSqrt(getAdd(getMultiply(arg0, arg0), W.ONE)))));
                        break;
                    case "acosh":
                        if (!CONFIG_HYP_INV) {
                            throw new Error("Unsupported Operation: acosh");
                        }
                        stack.push(getLn(getAdd(arg0, getSqrt(getSub(getMultiply(arg0, arg0), W.ONE)))));
                        break;
                    case "atanh":
                        if (!CONFIG_HYP_INV) {
                            throw new Error("Unsupported Operation: atanh");
                        }
                        stack.push(getDivide(getLn(getDivide(getAdd(W.ONE, arg0), getSub(W.ONE, arg0))), W.TWO));
                        break;
                }
            }
            catch (e) {
